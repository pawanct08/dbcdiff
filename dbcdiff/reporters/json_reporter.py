...
def write_json(entries: list[DiffEntry], fp: TextIO, pretty: bool = True,
    ...
    payload = {
        ...
        "summary": {
            "total":          len(entries),
            "critical":       sum(1 for e in entries if e.severity == Severity.BREAKING),
            "warning":        sum(1 for e in entries if e.severity == Severity.FUNCTIONAL),
            "info":           sum(1 for e in entries if e.severity == Severity.METADATA),
            ...
            "worst_severity": ms.short_label() if ms else "IDENTICAL",
        },
